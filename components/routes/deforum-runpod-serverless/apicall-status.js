async function ApiCallStatus(jobId) {
    const response = await fetch(`https://api.runpod.ai/v2/${process.env.DEFORUM_SLS_ENDPOINT_ID}/status/${jobId}`, {
        headers: {
    ...
    status.logs = logs;
    status.endpointId = process.env.DEFORUM_SLS_ENDPOINT_ID;
